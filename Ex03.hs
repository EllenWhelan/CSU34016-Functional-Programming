...
insBST a b (Branch leftSub x y rightSub)
	| a<x 	= Branch(insBST a b leftSub) x y rightSub
	| a>x	= Branch leftSub x y (insBST a b rightSub)
	|otherwise = Branch leftSub a b rightSub
...
bst2assoc (Branch leftSub a b rightSub) = concat[bst2assoc leftSub, [(a,b)], bst2assoc rightSub]
